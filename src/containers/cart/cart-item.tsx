...
const CartItem: React.FC<CartItemProps> = ({ item }) => {
  const { removeFromCart, qtyUpdate } = useCart();

  // const attributes = cartItems.find((i: any) => i.product === item.id);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col w-full overflow-hidden">
        ...
        <div className="text-sm text-gray-400 mb-2.5 space-x-1">
          <span>Unit Price : </span>
          <span className="space-x-2">
            {Number(item.regularPrice) > Number(item.price) && (
              <del>{item.regularPrice}</del>
            )}
            <ins className="text-base font-semibold text-black no-underline">
              {item.price}
            </ins>
          </span>
        </div>
        ...
        <div className="flex items-end justify-between">
          <Counter
            quantity={item?.qty}
            onIncrement={() => qtyUpdate("INCREMENT", item.id)}
            onDecrement={() => item.qty > 1 && qtyUpdate("DECREMENT", item.id)}
            variant="dark"
          />
