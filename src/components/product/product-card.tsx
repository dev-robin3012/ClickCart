    ...
    >
      ...
      >
        <Image
          // @ts-ignore
          src={
            product?.images?.length ? product?.images[0]?.src : placeholderImage
          }
          width={demoVariant === "ancient" ? 352 : +imgWidth}
          height={demoVariant === "ancient" ? 452 : +imgHeight}
          loading={imgLoading}
          quality={100}
          alt={product?.name || product.images[0].alt || "Product Image"}
          className={cn(
      ...
      >
        ...
        <div
          ...
          className="font-semibold [&>del]:text-gray-400 text-black [&>del]:text-base [&>ins]:no-underline text-lg mt-5"
        ></div>
      </div>
